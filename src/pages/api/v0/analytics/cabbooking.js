...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { vendorNames, vendorEmails, month, status, avpId, avpName } =
      req.query;
      ...
      if (vendorNames) {
        ...
        conditions.push({
          $or: [
            {
              cabOwnerName: {
                $in: names.map((n) => new RegExp(n.trim(), "i")),
              },
            },
            {
    ...
    const [
      ...
      // CabVendor documents
      CabVendor.find(
        vendorMatchConditions && Object.keys(vendorMatchConditions).length > 0
          ? vendorMatchConditions
          : {}
      ),
        ...
        avpUsers = (allEmployees || [])
          .filter((emp) => {
            ...
            if (Array.isArray(emp.roles) && emp.roles.length > 0) {
              return emp.roles.some((roleId) =>
                avpRoleIds.includes(String(roleId))
              );
            }
            return false;
          })
        ...
        avpUsers = (allEmployees || [])
          .filter((emp) => {
            return (
              emp &&
              emp.designation &&
              String(emp.designation).toLowerCase().includes("avp")
            );
          })
          .map((emp) => ({
            id: emp._id || emp.id,
    ...
    const allVendors = [
      ...vendorAnalytics.map((v) => {
        ...
        if (Array.isArray(v.bookings)) {
          paymentDue = v.bookings
            .filter(
              (b) => b.status && b.status.toLowerCase().includes("payment")
            )
