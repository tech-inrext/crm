...
async function handler(req, res) {
  try {
    ...
    const todayUsers = await Employee.countDocuments({
      createdAt: { $gte: todayStart, $lte: todayEnd },
    });
    const yesterdayUsers = await Employee.countDocuments({
      createdAt: { $gte: yestStart, $lte: yestEnd },
    });
    const beforeYesterdayUsers = await Employee.countDocuments({
      createdAt: { $gte: beforeYestStart, $lte: beforeYestEnd },
    });

    // Trend for Pending MoUs
    const todayPendingMou = await Employee.countDocuments({
      mouStatus: "Pending",
      createdAt: { $gte: todayStart, $lte: todayEnd },
    });
    const yesterdayPendingMou = await Employee.countDocuments({
      mouStatus: "Pending",
      createdAt: { $gte: yestStart, $lte: yestEnd },
    });
    const beforeYesterdayPendingMou = await Employee.countDocuments({
      mouStatus: "Pending",
      createdAt: { $gte: beforeYestStart, $lte: beforeYestEnd },
    });

    // Trend for Approved MoUs
    const todayApprovedMou = await Employee.countDocuments({
      mouStatus: "Approved",
      createdAt: { $gte: todayStart, $lte: todayEnd },
    });
    const yesterdayApprovedMou = await Employee.countDocuments({
      mouStatus: "Approved",
      createdAt: { $gte: yestStart, $lte: yestEnd },
    });
    const beforeYesterdayApprovedMou = await Employee.countDocuments({
      mouStatus: "Approved",
      createdAt: { $gte: beforeYestStart, $lte: beforeYestEnd },
    });
    ...
    const [employeeResults, mouResults] = await Promise.all([
