...
export async function getVendor({ vendorNames, vendorEmails }) {
  ...
  // Build match conditions for specific vendors
  ...
  const [
    ...
    Employee.aggregate([
      ...
      { $project: { _id: 1, name: { $ifNull: ["$cabOwnerName", "$driverName"] }, cabOwnerName: 1, driverName: 1, email: 1, phone: 1, vehicleType: 1, vehicleNumber: 1, totalBookings: 1, completedBookings: 1, pendingBookings: 1, totalEarnings: { $round: ["$totalEarnings", 2] }, avgFare: { $round: ["$avgFare", 2] } } },
    ...
    VendorBooking.aggregate([
      ...
      { $group: { _id: "$ownerName", totalBookings: { $sum: 1 }, completedBookings: { $sum: { $cond: [{ $regexMatch: { input: "$status", regex: /completed/i } }, 1, 0] } }, pendingBookings: { $sum: { $cond: [{ $regexMatch: { input: "$status", regex: /pending/i } }, 1, 0] } }, totalAmount: { $sum: "$amount" }, avgAmount: { $avg: "$amount" }, latestBooking: { $max: "$createdAt" } } },
      { $project: { name: "$_id", totalBookings: 1, completedBookings: 1, pendingBookings: 1, totalEarnings: "$totalAmount", avgFare: "$avgAmount", latestBooking: 1 } },
      { $sort: { totalBookings: -1 } },
    ]),
    CabVendor.distinct("cabOwnerName"),
  ]);

  const getBestVendorName = (v) => v.cabOwnerName || v.driverName || v.name || "Cab Driver";
  ...
  const uniqueVendors = allVendors.filter((v, i, arr) => i === arr.findIndex((x) => x.name.toLowerCase() === v.name.toLowerCase()));

  // Return all counts explicitly
  const totalCabVendors = vendorAnalytics.length;
  const totalCabVendorCollection = cabVendorsFromCollection.length;
  const totalGeneralVendors = vendorBookingStats.length;
  ...
  return {
    ...
    trend: { totalVendors: { today: todayVendors, yesterday: yesterdayVendors, beforeYesterday: beforeYesterdayVendors } },
  };
}
