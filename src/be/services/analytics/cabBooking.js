  ...
  async getCabBooking({ vendorNames, vendorEmails, month, status }) {
    ...
    const vendorAnalytics = await Employee.aggregate([
      ...
      { $addFields: {
          ...
          completedBookings: {
            $size: {
              $filter: { input: "$bookings", cond: { $regexMatch: { input: "$$this.status", regex: /completed/i } } }
            }
          },
          pendingBookings: {
            $size: {
              $filter: { input: "$bookings", cond: { $regexMatch: { input: "$$this.status", regex: /pending/i } } }
            }
          },
    ...
    const uniqueVendors = allVendors.filter((vendor, idx, self) => 
      idx === self.findIndex((v) => (v.name || "").toLowerCase() === (vendor.name || "").toLowerCase())
    );
