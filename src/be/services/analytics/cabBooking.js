...
export async function getCabBooking({ vendorNames, vendorEmails, month, status }) {
	await dbConnect();
	...
	if (vendorNames || vendorEmails) {
		...
		if (vendorNames) {
			...
			or.push({ cabOwnerName: { $in: names.map((n) => new RegExp(n.trim(), "i")) } });
			or.push({ driverName: { $in: names.map((n) => new RegExp(n.trim(), "i")) } });
		}
		if (vendorEmails) {
			const emails = Array.isArray(vendorEmails) ? vendorEmails : vendorEmails.split(",");
			or.push({ email: { $in: emails.map((e) => new RegExp(e.trim(), "i")) } });
		}
	...
	// Get vendor analytics (simplified)
	const vendorAnalytics = await Employee.aggregate([
		...
		{ $lookup: {
			...
			let: { vendorId: "$_id" },
			pipeline: [
				{ $match: { $expr: { $eq: ["$vendor", "$$vendorId"] }, ...filters } }
			],
			as: "bookings"
		} },
		{ $addFields: {
		...
		{ $project: {
			_id: 1, cabOwnerName: 1, driverName: 1, name: 1, email: 1, phone: 1, vehicleType: 1, vehicleNumber: 1,
			totalBookings: 1, completedBookings: 1, pendingBookings: 1,
			totalEarnings: { $round: ["$totalEarnings", 2] }, avgFare: { $round: ["$avgFare", 2] }
		} },
		{ $sort: { totalBookings: -1 } }
	]);

	// Get general vendor stats (simplified)
	const vendorBookingStats = await VendorBooking.aggregate([
		...
		{ $project: {
			name: "$_id", totalBookings: 1, completedBookings: 1, pendingBookings: 1,
			totalAmount: { $round: ["$totalAmount", 2] }, avgAmount: { $round: ["$avgAmount", 2] },
			latestBooking: 1, contactNumber: "$vendorInfo.contactNumber", vehicleType: "$vendorInfo.vehicleType",
			vehicleNumber: "$vendorInfo.vehicleNumber", location: "$vendorInfo.location", email: "$vendorInfo.email"
		} },
		{ $sort: { totalBookings: -1 } }
	]);

	// Compose vendor list (simplified)
	const getBestVendorName = (v) => v?.cabOwnerName || v?.driverName || v?.name || v?.email?.split("@")[0] || "Cab Driver";
	const allVendors = [
	...
	const uniqueVendors = allVendors.filter((vendor, idx, self) => idx === self.findIndex((v) => (v.name || "").toLowerCase() === (vendor.name || "").toLowerCase()));
