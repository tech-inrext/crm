...
export async function getCabBooking({ vendorNames, vendorEmails, month, status, avpId, avpName }) {
	await dbConnect();
	...
	if (month && month !== "all") {
		...
		bookingDateFilter = { createdAt: { $gte: startDate, $lte: endDate } };
	}
	// Status filter
	let statusFilter = {};
	...
	const [
		...
		Employee.aggregate([
			...
			{ $lookup: { from: "cabbookings", let: { vendorId: "$_id" }, pipeline: [
			...
			{ $project: {
				_id: 1, cabOwnerName: 1, driverName: 1, name: 1, firstName: 1, lastName: 1, email: 1, phone: 1, vehicleType: 1, vehicleNumber: 1, totalBookings: 1, completedBookings: 1, pendingBookings: 1, totalEarnings: { $round: ["$totalEarnings", 2] }, avgFare: { $round: ["$avgFare", 2] }, bookings: 1,
			} },
	...
	try {
		...
		if (avpRoleIds.length > 0) {
			avpUsers = (allEmployees || [])
				.filter((emp) => emp.role && avpRoleIds.includes(String(emp.role)))
				.map((emp) => ({
					id: emp._id || emp.id,
					_id: emp._id || emp.id,
			...
			avpUsers = (allEmployees || [])
				.filter((emp) => emp.designation && String(emp.designation).toLowerCase().includes("avp"))
				.map((emp) => ({
					id: emp._id || emp.id,
					_id: emp._id || emp.id,
					name: emp.name || [emp.firstName, emp.lastName].filter(Boolean).join(" ") || emp.email,
