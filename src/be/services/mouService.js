...
// Helper to format dates like: 10th day of July, 2025
...
// Helper to resolve standardized branch key from employee record
// Returns one of: 'Noida', 'Lucknow', 'Patna', 'Delhi' (fallback 'Noida')
const getBranchKey = (employee) => {
  ...
  // try substring matching for common variants
  for (const c of candidates) {
    if (!c) continue;
...
// Map branch keys to full address labels (keeps in sync with ForFreelancer.tsx)
...
const addWrappedText = (doc, text, options = {}) => {
  const {
    fontSize = 12,
    font = "Times-Roman",
    lineGap = 0,
...
const generateMOUPDF = async (employee, facilitatorSignatureUrl = "") => {
  ...
  // Map fields from Employee model to variables used in the PDF
  ...
  const slabPercentage =
    employee.slabPercentage ||
    employee.totalTop100Slab ||
    employee.percentageSlab ||
    "";
  ...
  // Date line (use employee branch/location instead of hardcoded city)
  ...
  // render address inline so next sentence begins immediately after
  doc
    .font("Times-Roman")
    .font("Times-Bold")
    .fontSize(12)
  ...
  // continue previous line: append this sentence immediately after address
  doc
    .font("Times-Roman")
    .font("Times-Roman")
    .fontSize(12)
  ...
  // continue on same line: append the percentage description
  doc
    .font("Times-Roman")
    .font("Times-Roman")
    .fontSize(12)
  ...
  considerationPoints.forEach((point) => addWrappedText(doc, point));
  ...
  // offset from currentY where signature images start (reduced to bring labels closer)
  const sigStartOffset = 8; // tightened further

  // Try to render facilitator's signature fetched from loggedinuserprofile API
      ...
      return await new Promise((resolve) => {
        const req = lib.get(
          ...
          (res) => {
            ...
            if (statusCode !== 200) return resolve(null);
            const chunks = [];
  ...
  // simple image fetch helper (no redirect handling)
  const fetchImageBufferSimple = async (url) => {
    try {
      const lib = url.startsWith("https://")
        ? require("https")
        : require("http");
  ...
  // fallback JSON fetch for internal next API - return null to avoid network calls by default
  const fetchJsonFromNextApi = async () => {
    // We intentionally avoid making external HTTP calls in normal flow.
    // If required later, implement using internal db access or server-side auth.
    return null;
  };

  try {
    // prefer facilitatorSignatureUrl passed by caller (server-side authenticated profile)
    let facilSigUrl = facilitatorSignatureUrl || "";

    if (!facilSigUrl) {
      // fallback: attempt to call internal API (defensive) - using same host is not necessary now
      const profile = await fetchJsonFromNextApi();
      facilSigUrl =
        (profile &&
          profile.data &&
          profile.data.employee &&
          profile.data.employee.signatureURL) ||
        (profile && profile.data && profile.data.signatureURL) ||
        "";
    }

    if (facilSigUrl) {
      const sigX = 50; // left side
      const sigMaxWidth = 150;
      const sigMaxHeight = 60;
      // reserve a block lower than text labels so images don't overlap
      const sigYStart = currentY + sigStartOffset;

      // attempt to render; allow remote or local paths
      try {
        doc.image(facilSigUrl, sigX, sigYStart, {
          fit: [sigMaxWidth, sigMaxHeight],
        });
      } catch (err) {
        // try fetching remote image buffer then render
        if (
    ...
    // swallow any errors so PDF generation continues
  }

  // render signature of logged-in user (prefer employee.loggedInUser.employee.signatureURL)
  const loggedUser =
    employee &&
  ...
  if (sigUrl) {
    try {
      const sigX = 420; // right side (Associate)
      const sigMaxWidth = 150;
      const sigMaxHeight = 80;
      const sigYStart = currentY + sigStartOffset; // align with facilitator image start

      // helper: fetch image buffer and follow redirects (up to maxRedirects)
      const fetchImageBuffer = async (url, maxRedirects = 5) => {
        let current = url;
        for (let i = 0; i <= maxRedirects; i++) {
            ...
            const req = lib.get(
              ...
              (res) => {
                ...
                if (statusCode >= 300 && statusCode < 400 && headers.location) {
                  // resolve with redirect location marker
                  res.resume();
          ...
          if (result && result.redirect) {
            ...
            if (/^https?:\/\//i.test(loc)) {
              ...
              // construct absolute URL from current
              try {
      ...
      // try direct rendering (works for local paths or already-buffer)
      try {
        doc.image(sigUrl, sigX, sigYStart, {
          fit: [sigMaxWidth, sigMaxHeight],
        });
      } catch (e) {
        // fallback: attempt to fetch remote https/http image into a buffer and render
  ...
  // Reserve vertical space for signature block and then place Associate labels below it
  const sigBlockHeight = Math.max(60, 80); // facilitator maxHeight =60, associate maxHeight=80
  const sigBlockPadding = 4; // tightened padding
  const afterSigY =
    currentY + sigStartOffset + sigBlockHeight + sigBlockPadding;

  // Right side - Associate/Freelancer (placed below signature images)
  doc
    .font("Times-Roman")
    .font("Times-Bold")
    .fontSize(12)
  ...
  // Left side - Authorized Person labels (placed below facilitator signature image)
  // align facilitator labels with associate labels (same baseline)
  const facilitatorLabelY = afterSigY; // match right-side labels baseline
  doc
    .font("Times-Roman")
    .font("Times-Bold")
    .fontSize(12)
