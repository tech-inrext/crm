...
const generateMOUPDF = async (mouData) => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ margin: 50 });
      const filename = `MOU_${
        mouData.associateId || mouData._id || "unknown"
      }.pdf`;
      ...
      const employeeId = mouData.associateId || mouData._id || "";
      const createdAt = mouData.createdAt;
      const employeeName =
        mouData.name ||
        mouData.fullname ||
        mouData.username ||
      ...
      const employeeAddress =
        mouData.address || mouData.currentAddress || "______________________";
      const slabPercentage =
        mouData.slabPercentage ||
        mouData.totalTop100Slab ||
        mouData.percentageSlab ||
        "";
      const employeeEmail = mouData.email || "";
      const employeePhone = mouData.phone || mouData.phoneNumber || "";
      ...
      // Associate ID on right
      doc
        .font("Times-Roman")
        .fontSize(12)
        .text(`Associate ID: ${employeeId || "______________________"}`, {
          align: "right",
        });
      doc.moveDown();
      ...
      doc
        ...
        .fontSize(12)
        .text(
          "This Memorandum of Understanding (i.e. MOU) is executed at Noida on ",
          { continued: true }
        );
      doc
        .font("Times-Roman")
      ...
      addWrappedText(
        doc,
        "is a company having its Co. office at D-4, 3rd Floor, Sector-10, Noida, Uttar Pradesh-201301, hereinafter referred to as the facilitator which expression unless repugnant to the context or subject shall mean and include its successors, representatives and assigns; i.e. the First Party."
      );
      ...
      doc
        ...
        .font("Times-Bold")
        .fontSize(12)
        .text(`${employeeName}`, { continued: true });
      doc
        .font("Times-Roman")
        .font("Times-Bold")
        .fontSize(12)
        .text(` ${employeeAddress}`, { continued: false });
      doc.moveDown();
      ...
      doc
        ...
        .font("Times-Bold")
        .fontSize(12)
        .text(`${slabPercentage}`, { continued: true });
      addWrappedText(
        doc,
      ...
      // Left side - Authorized Person
      doc
        .font("Times-Roman")
        .font("Times-Bold")
        .fontSize(12)
        .text(`Authorized Person`, 50, currentY, { align: "left" });
      const facilitatorLineY = currentY + 15;
      doc.text("M/s. Inrext Pvt. Ltd.", 50, facilitatorLineY, {
        align: "left",
      });
      doc.text("FACILITATOR", 50, facilitatorLineY + 15, { align: "left" });

      // Right side - Associate/Freelancer
      doc
        .font("Times-Roman")
        .font("Times-Bold")
        .fontSize(12)
        .text(`Associate`, 350, currentY, { align: "left" });
      doc.text(employeeName || "", 350, currentY + 15, { align: "left" });
      doc.text("ASSOCIATE", 350, currentY + 45, { align: "left" });
...
const sendApprovalConfirmationEmail = async (employeeId) => {
  const MOU_raw = require("../../models/Employee");
  const MOU = MOU_raw && MOU_raw.default ? MOU_raw.default : MOU_raw;
  const completeMOU = await MOU.findById(employeeId).lean();
  if (!completeMOU) throw new Error("Employee not found");

  const pdfPath = await generateMOUPDF(completeMOU);

  const transporter = createTransporter();

  const mailOptions = {
    from: process.env.EMAIL,
    to: completeMOU.email,
    subject: `MOU Approved - ${completeMOU.associateId || completeMOU._id}`,
    html: `<p>Dear ${completeMOU.username},</p><p>Your MOU has been approved. Please find attached PDF.</p>`,
    attachments: [
      {
...
export { generateMOUPDF, sendApprovalConfirmationEmail };
