...
const UserDetailsDialog: React.FC<UserDetailsDialogProps> = ({
    ...
    const DetailItem = ({ icon: Icon, label, value, isLink = false }: { icon: any, label: string, value: any, isLink?: boolean }) => (
            ...
            >
                <Box sx={{ p: { xs: 2.5, sm: 4 } }}>
                    ...
                    >
                        ...
                        >
                            {!user.avatarUrl && (user.name?.substring(0, 2).toUpperCase() || <Person />)}
                        </Avatar>
                                ...
                                <Box>
                                    ...
                                    <Box display="flex" gap={1} flexWrap="wrap" alignItems="center">
                                        <Chip
                                            ...
                                            size="small"
                                            color="primary"
                                            sx={{ textTransform: "capitalize", fontWeight: 700, height: 24, px: 1 }}
                                        />
                                        {user.isCabVendor && (
                                            <Chip
                                                label="Cab Vendor"
                    ...
                    <Box display="grid" gridTemplateColumns={{ xs: "1fr", md: "1fr 1fr" }} gap={3}>
                        ...
                        <Box sx={{ bgcolor: "#fff", p: 3, borderRadius: 2, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                            <Typography variant="overline" color="primary.main" fontWeight={800} sx={{ mb: 2.5, display: 'flex', alignItems: 'center', gap: 1, letterSpacing: 1.2, fontSize: '0.75rem' }}>
                            ...
                            <Box display="flex" flexDirection="column" gap={2.5}>
                                ...
                                <DetailItem icon={Phone} label="Alternate Phone" value={user.altPhone} />
                        ...
                        <Box sx={{ bgcolor: "#fff", p: 3, borderRadius: 2, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", height: 'fit-content' }}>
                            <Typography variant="overline" color="secondary.main" fontWeight={800} sx={{ mb: 2.5, display: 'flex', alignItems: 'center', gap: 1, letterSpacing: 1.2, fontSize: '0.75rem' }}>
                        ...
                        {/* Documents Card */}
                        <Box sx={{ bgcolor: "#fff", p: 3, borderRadius: 2, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                            <Typography variant="overline" color="warning.main" fontWeight={800} sx={{ mb: 2.5, display: 'flex', alignItems: 'center', gap: 1, letterSpacing: 1.2, fontSize: '0.75rem' }}>
                        ...
                        {/* Nominee Card */}
                        <Box sx={{ bgcolor: "#fff", p: 3, borderRadius: 2, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                            <Typography variant="overline" color="success.main" fontWeight={800} sx={{ mb: 2.5, display: 'flex', alignItems: 'center', gap: 1, letterSpacing: 1.2, fontSize: '0.75rem' }}>
                                <FamilyRestroom fontSize="small" />
                                NOMINEE DETAILS
                            </Typography>
                            <Box display="flex" flexDirection="column" gap={2.5}>
                                <DetailItem icon={Person} label="Name" value={user.nominee?.name} />
                                <DetailItem icon={Phone} label="Phone" value={user.nominee?.phone} />
                                <DetailItem icon={Work} label="Occupation" value={user.nominee?.occupation} />
                                <DetailItem icon={FamilyRestroom} label="Relation" value={user.nominee?.relation} />
                                <DetailItem icon={Person} label="Gender" value={user.nominee?.gender} />
                            </Box>
                        </Box>
