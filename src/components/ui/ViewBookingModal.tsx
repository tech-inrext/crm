...
const ViewBookingModal: React.FC<ViewBookingModalProps> = ({
  ...
  const getPlcTypeLabel = (type: string) => {
    switch (type) {
      case 'percentage': return 'PLC %';
      case 'per_sq_ft': return 'PLC / s.q.f.t';
      case 'per_sq_yard': return 'PLC / s.q.y.d';
      case 'unit': return 'PLC / unit';
      default: return 'N/A';
    }
  };
  ...
  const getPaymentDetails = () => {
    switch (booking.paymentMode) {
      case 'cheque':
        return booking.chequeNumber ? `Cheque No: ${booking.chequeNumber}` : 'N/A';
      case 'online':
        return booking.transactionId ? `Transaction ID: ${booking.transactionId}` : 'N/A';
      case 'cash':
        return booking.cashReceiptNumber ? `Receipt No: ${booking.cashReceiptNumber}` : 'N/A';
      default:
        return 'N/A';
    }
  };
  ...
  const getPanImageUrl = () => {
    ...
    return typeof booking.panImage === 'string' ? booking.panImage : booking.panImage.url;
  };
  ...
  const getAadharImageUrls = () => {
    ...
    return booking.aadharImage.map((img: any) => 
  ...
  const getFileNameFromUrl = (url: string, defaultName: string, index?: number) => {
    try {
      ...
      if (index !== undefined) {
        return `${defaultName.toLowerCase().replace(/\s+/g, '-')}-${index + 1}.${extension}`;
      }
      return `${defaultName.toLowerCase().replace(/\s+/g, '-')}.${extension}`;
    } catch {
      if (index !== undefined) {
        return `${defaultName.toLowerCase().replace(/\s+/g, '-')}-${index + 1}.jpg`;
      }
                  ...
                  <Tooltip title={`Download ${title}`}>
                    <Button
                      ...
                      onClick={() => downloadFile(
                        doc, 
                        getFileNameFromUrl(doc, type === 'pan' ? 'pan-card' : 'aadhar-card', index)
                      )}
                      fullWidth
                      sx={{ 
                        minWidth: 'auto',
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Box display="flex" alignItems="center" gap={1}>
            <Tooltip title="Download">
              <IconButton 
                onClick={() => selectedImage && downloadFile(
                  selectedImage, 
        ...
        <DialogContent dividers>
          <Box sx={{ p: 1 }}>
            ...
            <Section title="Project & Customer Details">
              ...
              <InfoRow label="Customer 1 Name" value={booking.customer1Name} half />
              <InfoRow label="Customer 2 Name" value={booking.customer2Name} half />
            ...
            <Section title="Unit Details">
              ...
              <InfoRow label="Area" value={booking.area ? `${booking.area} sq ft` : "N/A"} half />
            ...
            <Section title="Financial Details">
              <InfoRow label="Project Rate" value={formatCurrency(booking.projectRate)} half />
              
              {/* PLC Details - UPDATED */}
              <InfoRow label="PLC Type" value={getPlcTypeLabel(booking.plcType)} half />
              <InfoRow label="PLC Value" value={booking.plcValue} half />
              
              <InfoRow label="Other Charges" value={formatCurrency(booking.otherCharges1)} half />
              <InfoRow label="Company Discount" value={formatCurrency(booking.companyDiscount)} half />
              <InfoRow label="Company Logged In Rate" value={formatCurrency(booking.companyLoggedInRate)} half />
              <InfoRow label="Booking Amount" value={formatCurrency(booking.bookingAmount)} half />
              <InfoRow label="Net Sold COP Amount" value={formatCurrency(booking.netSoldCopAmount)} half />
            </Section>
            ...
            <Section title="Sales & Commission">
              <InfoRow label="Sales Person Discount BSP" value={formatCurrency(booking.salesPersonDiscountBSP)} half />
              <InfoRow label="Sales Person Discount PLC" value={formatCurrency(booking.salesPersonDiscountPLC)} half />
              <InfoRow label="Sales Person Discount Club" value={formatCurrency(booking.salesPersonDiscountClub)} half />
              <InfoRow label="Sales Person Discount Others" value={formatCurrency(booking.salesPersonDiscountOthers)} half />
              <InfoRow label="Slab Percentage" value={booking.slabPercentage} half />
              <InfoRow label="Total Discount From Commission" value={formatCurrency(booking.totalDiscountFromComm)} half />
              <InfoRow label="Net Applicable Commission" value={formatCurrency(booking.netApplicableComm)} half />
            </Section>
            ...
            <Section title="Transaction Details">
              ...
              <InfoRow label="Transaction Date" value={formatDate(booking.transactionDate)} half />
            ...
            <Section title="Team Details">
              <InfoRow label="Sales Person Name" value={booking.salesPersonName} half />
              <InfoRow label="Team Head Name" value={booking.teamHeadName} half />
              <InfoRow label="Team Leader Name" value={booking.teamLeaderName} half />
              <InfoRow label="Business Head" value={booking.businessHead} half />
            </Section>
