...
const budgetRanges = [
  ...
  {
    value: "1 Lakh to 10 Lakh",
    label: "1 Lakh to 10 Lakh",
    min: 100000,
    max: 1000000,
  },
  {
    value: "10 Lakh to 20 Lakh",
    label: "10 Lakh to 20 Lakh",
    min: 1000000,
    max: 2000000,
  },
  {
    value: "20 Lakh to 30 Lakh",
    label: "20 Lakh to 30 Lakh",
    min: 2000000,
    max: 3000000,
  },
  {
    value: "30 Lakh to 50 Lakh",
    label: "30 Lakh to 50 Lakh",
    min: 3000000,
    max: 5000000,
  },
  {
  ...
  useEffect(() => {
    ...
    const loadProperties = async () => {
      ...
      try {
        ...
        const items: PropertyApiItem[] = Array.isArray(json.data)
          ? json.data
          : [];
        const options = items
          .map((item) => {
  ...
  useEffect(() => {
    ...
    const loadSubProperties = async () => {
      ...
      try {
        ...
        const items: SubPropertyItem[] = Array.isArray(json.data)
          ? json.data
          : [];
        const uniqueTypes = Array.from(
          new Set(
            items.map((item) => item.propertyType).filter(Boolean) as string[]
          )
        );
        ...
        if (isMounted) {
          ...
          if (options.length === 1) {
            ...
            !options.find((opt) => opt.value === values.propertyType)
          ) {
